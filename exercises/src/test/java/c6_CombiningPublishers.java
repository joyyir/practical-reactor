...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    @Test
    void test_flatMap_flatMapSequential_concatMap() {
        Function<Integer, Publisher<? extends Integer>> mapper =
            value -> Mono.fromCallable(() -> value)
                         .delaySubscription(Duration.ofSeconds(ThreadLocalRandom.current().nextInt(1, 5)))
                         .doOnSubscribe(subscription -> System.out.println("Subscribe " + value + ", thread: " + Thread.currentThread().getName()));
        Consumer<Integer> onNext = x -> System.out.println("onNext: " + x + ", thread: " + Thread.currentThread().getName());
    ...
    public void instant_search() {
        Flux<String> suggestions = userSearchInput()
            .switchMap(x -> autoComplete(x)); // TODO : jyjang - switchMap을 이렇게 쓰는구나
    ...
    public void acid_durability() {
        Flux<String> committedTasksIds = tasksToExecute()
            .concatMap(taskMono -> taskMono.flatMap(taskId -> commitTask(taskId).thenReturn(taskId))); // TODO : jyjang - 어렵다... 나중에 다시 해보기
...
//                                     .map(objects -> {
//                                         Chassis chassis = objects.getT1();
//                                         Engine engine = objects.getT2();
//                                         return new Car(chassis, engine);
//                                     })
//                                     .doOnNext(car -> System.out.println("Car produced!"));
